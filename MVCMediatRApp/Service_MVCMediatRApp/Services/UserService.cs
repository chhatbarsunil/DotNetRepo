...
namespace Service_MVCMediatRApp.Services
{
    ...
    {
        private readonly MVCMediatRAppDbContext _dbContext;

        public UserService(MVCMediatRAppDbContext dbContext)
        {
        ...
        public async Task<UpdateUserRequestModel> GetUserUpdateById(int id)
        {
            User user = new();
            // Option 1: Using Where clause and FirstOrDefault
            var result = await _dbContext.Users.Where(u => u.Id == id).FirstOrDefaultAsync();
            ...
            //Map using Automapper User to UserViewModel
            UpdateUserRequestModel updateUserRequestModel = new();
            updateUserRequestModel.Id = user.Id;
            updateUserRequestModel.UserName = user.UserName;
            updateUserRequestModel.Email = user.Email;
            updateUserRequestModel.Designation = user.Designation;
            updateUserRequestModel.ReportingManager = user.ReportingManager; ;
            updateUserRequestModel.Roles = user.Roles;
            updateUserRequestModel.Approver = user.Approver;
            updateUserRequestModel.DepartmentName = user.DepartmentName;
            updateUserRequestModel.IsVendor = user.IsVendor;
            updateUserRequestModel.Phone = user.Phone;
            updateUserRequestModel.Password= user.Password;
            return updateUserRequestModel;
        }

        public async Task<UpdateUserRequestModel> UpdateUser(UpdateUserRequestModel updateUserRequestModel)
        {
            //User user = new User();
